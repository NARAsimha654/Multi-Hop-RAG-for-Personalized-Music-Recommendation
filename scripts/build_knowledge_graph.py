...
from collections import defaultdict # For artist-genre mapping
...
def insert_genres(driver, genre_list):
    ...
    db_name = "db-1"
    query = "UNWIND $genres AS gp MERGE (g:Genre {name: gp.name})"
...
def create_same_as_rels(driver, direct_matches_df, fuzzy_matches_df):
    ...
    for match_tuple in unique_match_tuples:
        ...
        if fuzzy_match_dict: final_match_data.append(fuzzy_match_dict)
            ...
            if direct_match_dict: final_match_data.append(direct_match_dict)
...
def create_cooccurrence_rels_iterative(driver, playlists_df):
    ...
    if playlists_df is None or playlists_df.empty or 'pid' not in playlists_df.columns:
        print("  Error: Playlists DataFrame is missing or invalid."); return
    try: playlist_pids = playlists_df['pid'].dropna().astype(int).unique().tolist()
    except ValueError: print("Error: Could not convert playlist PIDs to integers."); return
    if not playlist_pids: print("  No playlist PIDs found to process."); return

    print(f"  Processing {len(playlist_pids)} playlists one by one (this might take a while)...")
    start_tm = time.time(); processed_count = 0; error_count = 0
    query = """
    MATCH (p:Playlist {pid: $pid})<-[:APPEARS_IN]-(t1:MpdTrack)
    WITH p, collect(t1) as tracks_in_playlist
    UNWIND tracks_in_playlist as t1
    ...
    with driver.session(database=db_name) as session:
        for pid in tqdm(playlist_pids, desc="Processing Playlists for Co-occurrence", unit="playlist"):
            try:
                ...
                error_count += 1; tqdm.write(f"\n  Error processing playlist PID {pid} for co-occurrence: {e}")
    end_tm = time.time()
    print(f"\n  Finished processing {processed_count} playlists for co-occurrence in {end_tm - start_tm:.2f} seconds.")
    if error_count > 0: print(f"  Encountered errors processing {error_count} playlists.")
    print(f"  :CO_OCCURS_WITH relationship creation/update process completed.")

# --- NEW Function to Create Approximate Track-Genre Relationships ---
def create_track_genre_rels(driver, mpd_tracks_df, artist_genre_map):
    """
    Creates :HAS_GENRE relationship between MpdTrack and Genre nodes
    based on the artist's primary genre found in GTZAN data.
    """
    print("\nStep: Creating (MpdTrack)-[:HAS_GENRE]->(Genre) relationships (approximate)...")
    db_name = "db-1"

    # Prepare data: list of {'track_uri': uri, 'genre_name': genre}
    rel_data = []
    # Create a set of artists from the map keys for faster lookup
    mappable_artists = set(artist_genre_map.keys())

    print("  Mapping tracks to genres based on artist...")
    # Iterate through MPD tracks, check if artist is in the map
    for index, row in mpd_tracks_df[['track_uri', 'artist_name']].dropna().iterrows():
        artist = row['artist_name']
        if artist in mappable_artists:
            genre = artist_genre_map[artist] # Get genre from map
            rel_data.append({'track_uri': row['track_uri'], 'genre_name': genre})

    if not rel_data:
        print("  No tracks found whose artists have a mapped genre from GTZAN.")
        return

    batch_size = 5000
    total_rels = len(rel_data)
    print(f"  Preparing to create {total_rels} :HAS_GENRE relationships in batches of {batch_size}...")

    # Cypher query: MATCH Track, MATCH Genre, MERGE relationship
    query = """
    UNWIND $rels AS rp
    MATCH (t:MpdTrack {track_uri: rp.track_uri})
    MATCH (g:Genre {name: rp.genre_name})
    MERGE (t)-[:HAS_GENRE]->(g)
    """
    with driver.session(database=db_name) as session:
        for i in range(0, total_rels, batch_size):
            batch = rel_data[i:min(i + batch_size, total_rels)]
            # print(f"    Creating relationship batch {i // batch_size + 1}/{(total_rels + batch_size - 1) // batch_size} ({len(batch)} relationships)...") # Reduced verbosity
            try:
                session.execute_write(run_cypher_query, query, parameters={'rels': batch})
            except Exception as e:
                print(f"  Error creating Track-Genre relationships batch {i}: {e}")
    print(f"  :HAS_GENRE relationship creation completed.")
...
if __name__ == "__main__":
    ...
    except Exception as e: print(f"An unexpected error occurred during file loading: {e}"); sys.exit(1)

    # 3. Insert Nodes
    print("\n--- Inserting/Merging Nodes ---")
    artist_genre_map = {} # Initialize artist genre map
    ...
    if gtzan_features_df is not None and 'genre' in gtzan_features_df.columns:
        ...
        # --- Create Artist -> Genre Mapping from GTZAN ---
        # This assumes GTZAN features has 'filename' which can be mapped to artist
        # NOTE: GTZAN filenames usually DON'T contain artist names directly.
        # This part needs adjustment based on how you can link GTZAN files to artists.
        # For now, we'll create an empty map as a placeholder.
        # If you had a mapping like gtzan_artist_map = {'blues.00000.wav': 'Artist Name', ...}
        # you could use that here.
        print("  Creating Artist->Genre map (Placeholder - Requires GTZAN->Artist mapping)...")
        # Placeholder: You need a way to map GTZAN files/features to artist names
        # Example (if you had such a mapping):
        # for index, row in gtzan_features_df.iterrows():
        #    artist = get_artist_from_gtzan_filename(row['filename']) # Implement this function
        #    if artist and artist not in artist_genre_map: # Assign first genre found
        #         artist_genre_map[artist] = row['genre']
        if not artist_genre_map:
             print("  Warning: Could not create artist->genre map from GTZAN data.")

    else: print("\nSkipping Genre node insertion and Artist->Genre mapping.")
    ...
    if not mpd_playlists_df.empty and not mpd_playlist_tracks_df.empty:
        create_cooccurrence_rels_iterative(driver, mpd_playlists_df)
    else: print("Skipping CO_OCCURS_WITH relationships.")

    # --- Create Track-Genre Relationships ---
    if not mpd_tracks_df.empty and artist_genre_map:
        create_track_genre_rels(driver, mpd_tracks_df, artist_genre_map)
    else:
        print("Skipping Track-Genre relationships (missing track data or artist->genre map).")
